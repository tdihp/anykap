...
async def test_shelltask_simple(hq, hqtask):
    ...
    set(child.name for child in my_artifact.path.iterdir()) >= {
        "sh.stdout",
        "sh.stderr",
        "sh.result",
        "anykap.log",
...
async def test_shelltask_notify(hq, hqtask):
    ...
    shellresult = [
        await asyncio.wait_for(q.get(), timeout=3) for i in range(len(myresult))
    ]
...
async def test_shelltask_timeout(hq, hqtask):
    ...
    q.task_done()
    assert (
        result["status"] == -signal.SIGTERM and result["task_name"] == "sleep-forever"
    )
    ...
    q.task_done()
    assert (
        result["status"] == -signal.SIGKILL
        and result["task_name"] == "sleep-block-sigterm"
    )
