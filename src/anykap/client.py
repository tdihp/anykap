...
def get_covered(parser, args, kubectl_args):
    ...
    found = [
        ...
        and any(
            c["name"] == "anykap" for c in d["spec"]["template"]["spec"]["containers"]
        )
    ]
    try:
        (found_ds,) = found
    ...
    output = json.loads(
        kubectl(
            "get", "pods", "-o", "json", "-n", namespace, "-l", selector, *kubectl_args
        )
    )
...
def cmd_tasks(parser, args, kubectl_args):
    ...
    for node, pod in iter_nodes(parser, args, pods):
        try:
            result = repl_req(
                ("-n", namespace, pod) + tuple(kubectl_args), query, **req_kw
            )
            assert len(result) == 1
